...
<div class="form-group{{ $errors->has('district_id') ? 'has-error' : ''}}">
    ...
    <select name="region_id" id="regions" class="form-control " required >
        ...
        <option @isset($plantedproduct)
            {{ $plantedproduct->region_id==$region->id ? 'selected' : '' }} @endisset value="{{ $region->id }}" >{{ $region->name }}
        </option>
        @endforeach
    </select>
...
<div class="form-group{{ $errors->has('district_id') ? 'has-error' : ''}}">
    ...
    <select name="district_id" id="districts" class="form-control" required>
        ...
        <option @isset($plantedproduct)
            {{ $plantedproduct->district_id==$district->id ? 'selected' : '' }} @endisset value="{{ $district->id }}" data-region_id={{ $district->region->id }}>{{ $district->name }}
        </option>
        @endforeach
    </select>
...
<div class="form-group{{ $errors->has('district_id') ? 'has-error' : ''}}">
    ...
    <select name="quarter_id" id="quarters" class="form-control" required>
        ...
        <option @isset($plantedproduct)
            {{ $plantedproduct->quarter_id==$quarter->id ? 'selected' : '' }} @endisset value="{{ $quarter->id }}" data-district_id={{ $quarter->district->id }}>{{ $quarter->name }}
        </option>
        @endforeach
    </select>
...
<div class="form-group{{ $errors->has('district_id') ? 'has-error' : ''}}">
    ...
    <select name="street_id" id="streets" class="form-control" required>
        ...
        <option @isset($plantedproduct)
            {{ $plantedproduct->street_id==$street->id ? 'selected' : '' }} @endisset value="{{ $street->id }}" data-street_id={{ $street->quarter->id }}>{{ $street->name }}
        </option>
        @endforeach
    </select>
...
<div class="form-group{{ $errors->has('firm_id') ? 'has-error' : ''}}">
    ...
    {!! $errors->first('firm_id', '<p class="help-block">:message</p>') !!}
</div>
...
@if($formMode=='edit')
    <div class="form-group cat_products">
        <div class="col-xs-3 cat_products_item">
            <label for="">{{ $plantedproduct->product->name }} ({{ $plantedproduct->product->segment_short }})</label>
            <input type="number" name="amount" class="form-control" placeholder="Miqdorni kiriting..." value="{{ $plantedproduct->amount }}" required>
        </div>
    </div>
@else
    <div class="form-group{{ $errors->has('amount') ? 'has-error' : ''}} cat_products">
        @foreach ($category_products as $cat_product)
        <div class="col-xs-3 cat_products_item">
            <label for="">{{ $cat_product->name }} (sotix)</label>
